...
class Intel:
    def __init__(self, server=config['redis']['ip'], **kwargs):
        self.kwargs = kwargs
        self.rserver = redis.Redis(host=server, port=config['redis']['port'], db=0)
        self.cache = {
            'virustotal': config['cache']['virustotal'],
            'greynoise': config['cache']['greynoise'],
            'xforce': config['cache']['xforce'],
            'hybridanalysis': config['cache']['hybridanalysis']
        }
